...
msg_result:    .asciiz "El mayor numero es: "                                # Mensaje para mostrar el resultado
...
# Leer el primer valor y asumir que es el máximo inicial
read_values:               # Inicio de la lectura de valores
    ...
    move $t1, $v0          # $t1 = max actual
    li $t2, 1              # Contador i = 1 (ya leímos 1 numero)

# Bucle para leer los (n-1) restantes y actualizar el máximo
loop_read:                 # Bucle principal de lectura/comparación
    ...
    # Comparar $t3 con el max actual ($t1)
    ble $t3, $t1, no_update   # Si $t3 <= $t1, no actualizar
    move $t1, $t3          # Si $t3 > $t1, actualizar max = $t3
no_update:                 # No se actualizó el máximo

    addi $t2, $t2, 1       # i++
    j loop_read            # Repetir

# Mostrar el resultado
show_result:               # Mostrar el máximo encontrado
    li $v0, 4              # Syscall 4: imprimir cadena
    la $a0, msg_result     # Mensaje "El mayor numero es: "
    syscall                # Imprimir mensaje

    li $v0, 1              # Syscall 1: imprimir entero
    move $a0, $t1          # Pasar el máximo en $t1
    syscall                # Imprimir valor del máximo
